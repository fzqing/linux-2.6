...
static int emac_poll_rx(void *param, int budget)
{
	...
	while (budget > 0) {
		...
		/*
		 * rx_slot needs to be updated now, in case we are interrupted
		 * by netpoll.  Upon return, use the new value.
		 */
		dev->rx_slot = (slot + 1) % NUM_RX_BUFF;
...
poll_ctrl(struct net_device *dev)
{
	...
	if (get_mal_dcrn(mal, MAL_CFG) & MAL_CFG_EOPIE) {
