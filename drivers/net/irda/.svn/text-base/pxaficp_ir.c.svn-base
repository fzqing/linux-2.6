...
static int pxa_irda_start(struct net_device *dev)
{
	...
	err = -ENOMEM;
	si->dma_rx_buff = dma_alloc_coherent(NULL, IRDA_FRAME_SIZE_LIMIT, &si->dma_rx_buff_phy, GFP_KERNEL );
	if (!si->dma_rx_buff)
...
static int pxa_irda_stop(struct net_device *dev)
{
	...
	if (si->dma_rx_buff) dma_free_coherent( NULL, IRDA_FRAME_SIZE_LIMIT, si->dma_tx_buff, si->dma_tx_buff_phy);
