			    ...
			    unsigned int level)
{
	...
	/* DMA Data Setup */
	ide_t0 = ((palm_bk3710_udmatimings[level].cycletime/2) / ide_palm_clk);
	ide_tenv = (palm_bk3710_udmatimings[level].envtime / ide_palm_clk);
	ide_trp = (palm_bk3710_udmatimings[level].rptime / ide_palm_clk);
			   ...
			   unsigned int cycletime, unsigned int mode)
{
	...
	ide_tkw = palm_bk3710_dmatimings[mode].recoverytime / ide_palm_clk;
...
static int palm_bk3710_hostdma(ide_drive_t * drive, u8 xferspeed)
{
	...
	if (nspeed != -1) {
		...
		retval = ide_config_drive_speed(drive, speed);
...
static void palm_bk3710_tune_drive(ide_drive_t * drive, u8 pio)
{
	...
	int is_slave = (&hwif->drives[1] == drive);
	unsigned int xfermode = 0;
	
	/* Get the best PIO Mode supported by the drive
	 * Obtain the drive PIO data for tuning the Palm Chip registers
	 */
	pio = ide_get_best_pio_mode(drive, pio, 5, &piodata);
