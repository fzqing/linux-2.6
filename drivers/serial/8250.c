...
#include <asm/rmi/iomap.h>
		{
			phoenix_reg_t *mmio =
				(phoenix_reg_t *)(up->port.membase + offset);
			return *mmio;
		}
#elif CONFIG_CPU_CAVIUM_OCTEON
...
static void autoconfig_16550a(struct uart_8250_port *up)
{
	...
	/* TSI108 UARTS are incorrectly detected as XSCALE by this bogus code */
...
//	save_flags(flags); cli();
	...
	if ((up->capabilities & UART_CAP_UUE) || (up->port.type == PORT_XSCALE))
...
serial8250_handle_port(struct uart_8250_port *up, struct pt_regs *regs)
{
	...
	static int iir_rdi_workaround_enable = 0;

	DEBUG_INTR("status = %x...", status);

	iir = serial_inp(up, UART_IIR);		/* read IRR first ... */
	status = serial_inp(up, UART_LSR);	/* ... then read LSR */

	if (status & UART_LSR_DR) {
		if (!iir_rdi_workaround_enable || (iir & UART_IIR_RDI))
			receive_chars(up, &status, regs);
	} else if ((iir & UART_IIR_ID) == UART_IIR_RDI) {
		/*
		 * The UART sets UART_IIR_RDI *even* if the received data
		 * has been read out from the FIFO before the timeout occurs.
		 * And as the Rx FIFO is empty in this case, there is no
		 * other way to clear UART_IIR_RDI but to reset the RX FIFO.
		 * 
		 * The case was encountered on 
		 * -- Motorola PrAMC 7201 (Intel 6300ESB I/O Controller Hub)
		 * -- Motorola ATCA-C110
		 */
		iir_rdi_workaround_enable = 1;
		if (up->capabilities & UART_CAP_FIFO) {
			unsigned char fcr = uart_config[up->port.type].fcr;
			serial_outp(up, UART_FCR, fcr | UART_FCR_CLEAR_RCVR);
		}
	}
