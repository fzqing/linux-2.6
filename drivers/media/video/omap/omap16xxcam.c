...
omap16xx_cam_init(void)
{
	...
	if (machine_is_omap_h3())
		omap_writew(omap_readw(OMAP16XX_ARM_IDLECT3) | (1 << EN_TC2CK),
			OMAP16XX_ARM_IDLECT3);
	else {
		omap_writew(omap_readw(ARM_IDLECT2) | (1 << EN_PERCK), ARM_IDLECT2);
...
omap16xxcam_open(void *priv)
{
	...
	if ((ret = request_irq(INT_CAMERA, omap16xx_cam_isr, 0, 
#else
