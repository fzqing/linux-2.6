...
struct gs_dev {
	...
	spinlock_t		send_lock;	/* mutual exclusion of gs_send in gs_write and gs_write_complete */
...
static int gs_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	...
	spin_lock_irqsave(&gs_device->send_lock, flags);
	gs_send(gs_device);
	spin_unlock_irqrestore(&gs_device->send_lock, flags);
...
requeue:
		...
		if (spin_trylock(&dev->send_lock)) {
			gs_send(dev);
			spin_unlock(&dev->send_lock);
		}
