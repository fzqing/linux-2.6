...
static int omapfb_suspend(struct device *dev, u32 state, u32 level);
...
{
	do_imageblit(fbi, image, 0);
}

/* Transparent image blit, using image->bg_color as the color key */
static void omapfb_transparent_blit(struct fb_info *fbi,
				    const struct fb_image *image)
{
	do_imageblit(fbi, image, COPY_MODE_TRANSPARENT);
}
...
static void set_fb_fix(struct omapfb_device *fbdev)
{
	...
	if (var->accel_flags == FB_ACCELF_TEXT && var->bits_per_pixel >= 8) {
		fbops->fb_fillrect = omapfb_fillrect;
		fbops->fb_copyarea = omapfb_copyarea;
		fbops->fb_imageblit = omapfb_imageblit;
	} else {
	...
	if (fbdev->frame1_org > fbdev->lcddma_mem_size)
		fbdev->frame1_org = 0;
 ...
 */
static int omapfb_select_frame(struct fb_info *fbi, unsigned int vis_idx,
			       unsigned int src_idx, unsigned int dst_idx)
{
	struct omapfb_device *fbdev = (struct omapfb_device *)fbi->par;

	if ((vis_idx | src_idx | dst_idx) & ~1)
		return -EINVAL;
	if (((vis_idx | src_idx | dst_idx) & 1) && !fbdev->frame1_org)
		return -EINVAL;
				...
				struct fb_update_window *win)
{
	...
	if (!win->width || !win->height)
		return 0;
	return fbdev->ctrl->update_window(fbdev, win);
}
			...
			struct fb_info *fbi)
{
	...
	case OMAPFB_FILLRECT:
		if (copy_from_user(&p.rect, (void *)arg, sizeof(p.rect)))
			r = -EFAULT;
		else
			ops->fb_fillrect(fbi, &p.rect);
		break;
	case OMAPFB_COPYAREA:
		if (copy_from_user(&p.area, (void *)arg, sizeof(p.area)))
			r = -EFAULT;
		else
			ops->fb_copyarea(fbi, &p.area);
		break;
	case OMAPFB_IMAGEBLIT:
		if (copy_from_user(&p.image, (void *)arg, sizeof(p.image)))
			r = -EFAULT;
		else
			ops->fb_imageblit(fbi, &p.image);
		break;
	case OMAPFB_TRANSPARENT_BLIT:
		if (copy_from_user(&p.image, (void *)arg, sizeof(p.image)))
			r = -EFAULT;
		else
			omapfb_transparent_blit(fbi, &p.image);
		break;
	case OMAPFB_MIRROR:
		if (get_user(p.mirror, (int *)arg))
			r = -EFAULT;
		else
			omapfb_mirror(fbi, p.mirror);
		break;
	case OMAPFB_SCALE:
		if (copy_from_user(&p.scale, (void *)arg, sizeof(p.scale)))
			r = -EFAULT;
		else
			r = omapfb_scale(fbi, p.scale.xscale, p.scale.yscale);
		break;
	case OMAPFB_SELECT_VIS_FRAME:
		if (get_user(p.frame_idx, (int *)arg))
			r = -EFAULT;
		else
			r = omapfb_select_frame(fbi, p.frame_idx,
						fbdev->src_frame_org,
						fbdev->dst_frame_org);
		break;
	case OMAPFB_SELECT_SRC_FRAME:
		if (get_user(p.frame_idx, (int *)arg))
			...
			r = omapfb_select_frame(fbi, fbdev->vis_frame_org,
						p.frame_idx,
						fbdev->dst_frame_org);
		break;
	...
	case OMAPFB_GET_FRAME_OFFSET:
		if (copy_from_user(&p.frame_offset, (void *)arg,
				   sizeof(p.frame_offset)))
			r = -EFAULT;
		else {
			r = omapfb_get_frame_offset(fbi, &p.frame_offset);
			if (copy_to_user((void *)arg, &p.frame_offset,
					 sizeof(p.frame_offset)))
				r = -EFAULT;
		}
		break;
...
//		r = -EINVAL;
	...
	case OMAPFB_SET_UPDATE_MODE:
		if (get_user(p.update_mode, (int *)arg))
			r = -EFAULT;
		else
	...
	case OMAPFB_GET_UPDATE_MODE:
		...
		if (put_user(p.update_mode, (enum fb_update_mode *)arg))
			r = -EFAULT;
		break;
	case OMAPFB_UPDATE_WINDOW:
		if (copy_from_user(&p.update_window, (void *)arg,
				   sizeof(p.update_window)))
			r = -EFAULT;
		else
			r = omapfb_update_window(fbi, &p.update_window);
		break;
	case OMAPFB_GET_CAPS:
		p.caps = omapfb_get_caps(fbi);
		if (put_user(p.caps, (unsigned long *)arg))
			r = -EFAULT;
		break;
	default:
		r = -EINVAL;
...
static int fbinfo_init(struct omapfb_device *fbdev)
{
	...
	info->screen_base = (char *) fbdev->lcddma_base + fbdev->frame0_org;
...
int __init omapfb_setup(char *options)
{
	char *this_opt = NULL;
	int r = 0;
