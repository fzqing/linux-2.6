...
/* Our I/O buffers must be larger than the number of register bytes we
 * have. */
#if BUFMAX < NUMREGBYTES
#error "We have more register space to send than buffer space."
#endif
...
static char *write_mem_msg(int binary)
{
	...
	if (kgdb_hex2long(&ptr, &addr) > 0 && *(ptr++) == ',' &&
		...
		if ((CACHE_FLUSH_IS_SAFE) && length > 0)
	...
	while (1) {
		...
		case 'q':
			...
			case 'f':
				...
				/* 22 threads will give us 384 bytes in
				 * this packet.  GDB has a minimum of
				 * 400 bytes for the buffer. */
				for (i = 0; i < 22 && threadid < pid_max +
 ...
 * breakpoint().
 */
static int kgdb_panic_notify(struct notifier_block *self, unsigned long cmd, void *ptr)
{
	breakpoint();
...
static void __init kgdb_internal_init(void)
{
	...
	/* Register for panics(). */
	notifier_chain_register(&panic_notifier_list, &kgdb_panic_notifier);
