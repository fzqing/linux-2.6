...
static long tstojiffie(struct timespec *tp, int res, u64 *jiff)
{
	...
	dividend = raw_jiff;
	rem = do_div(dividend, (unsigned long)res_jiff);
	raw_jiff -= rem;
...
#ifdef CONFIG_HIGH_RES_TIMERS
		/*
		 * In case of HIGH_RES_TIMERS, we should use the arch_cycles
		 * rather than simply adding the clock->res to the requested
		 * time to account for the time starting at mid jiffie.
		 * Because we can know the accurate current time by 
		 * get_arch_cycles().
		 */
		oc.tv_nsec += arch_cycle_to_nsec(arch_cycle);
#else
