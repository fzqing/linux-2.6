 ...
 *
 **************************************************************************************/
void omap_sound_dma_link_lch(void *data)
{
 ...
 *
 **************************************************************************************/
void omap_sound_dma_unlink_lch(void *data)
{
	audio_stream_t *s = (audio_stream_t *) data;
	int *chan = s->lch;
	int i;
...
static void sound_dma_irq_handler(int sound_curr_lch, u16 ch_status, void *data)
{
	...
	if (AUDIO_QUEUE_LAST(s))
		audio_stop_dma(s);
#ifdef CONFIG_PREEMPT_RT
	AUDIO_INCREMENT_HEAD(s);        /* Empty the queue here for RT case*/
#endif
