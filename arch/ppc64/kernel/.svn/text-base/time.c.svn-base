...
int hr_set_dec(unsigned long count) {
	...
	if (time_before(count, get_tb()))
		/* missed it */
		return 1;

	/* ok, we've got a shot */
	set_dec(count - get_tb());

	if (time_after(count, get_tb())) {
		hr_tick = 1;
		return 0; /* made it */
	}
...
/* 
 *	Do not call this when using HRT. This causes the time to skew and the
 *	nanosleep to wake up too late.
 */ 
...
#else
/* 
 * for the do_gettimeofday() function we must check the xtime_locked
 * due to do_gettimeofday being called in the timer interrupt as well 
 * as non-irq context. This ensure that we don't run into a deadlock
 * while attempting to access the xtime via the read sequence lock
 * We do not need a lock while we are in timer interrupt due to the write
 * lock is being held. However, if we do attempt to acquire the read "lock" 
 * while write lock is being held then we would deadlock. 
 *
 * Also, it is necessary to provide the method below for HRT in order for
 * the gettimeofday() and clock_gettime() results to be identical. Otherwise
 * they are different.
 */ 
void do_gettimeofday(struct timeval *tv)
{
	...
	} while(!xtime_locked && read_seqretry(&xtime_lock, seq));		
