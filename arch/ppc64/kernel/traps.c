...
asmlinkage void trace_real_syscall_entry(struct pt_regs *regs)
{
	...
	unsigned long addr;
	unsigned int  addr32;
	int user32 = (regs->msr & MSR_SF) ? 0 : 1; /* 32- or 64-bit userspace? */
	...
	/* Set the MSB of the syscall ID for 32-bit syscalls */
	if (user32)
		trace_syscall_event.syscall_id |= 0x8000;

	/* Are we in the kernel (this is a kernel thread)? */
	if (!user_mode(regs))
		/* Don't go divining anywhere */
	...
	/* Do we have to search for an IP address range */
	if (!(use_depth == 1 || use_bounds == 1))
		goto trace_syscall_end;

	/*
	 * Start at the top of the stack, skipping over the first stack
	 * frame as the return address isn't valid
	 */
	addr = regs->gpr[1];
	if (user32)
		addr &= 0xFFFFFFFF;
	...
	do {
		/* Skip forward to the next stack frame */
		if (user32) {
			if (get_user(addr32, stack))
				break;
			addr = addr32;
		} else if (get_user(addr, stack))
			break;

		stack = (unsigned long *) addr;

		/* Read the IP from this stack frame */
		if (user32) {
			if (get_user(addr32, (unsigned int *)stack + 1))
				break;
			addr = addr32;
		} else if (get_user(addr, stack + 2))
			break;
		...
		if ((addr > current->mm->start_code) &&
			...
			if ((use_depth == 1 && depth == seek_depth) ||
				...
				/* Set the address and we're done if so */
				trace_syscall_event.address = addr;
				break;
			} else /* dig deeper... */
				depth++;
		}
	} while (1); /* keep going until we reach the end of the process' stack... */
trace_syscall_end:
	/* Trace the event */
