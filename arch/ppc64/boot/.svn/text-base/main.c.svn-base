...
void start(unsigned long a1, unsigned long a2, void *promptr)
{
	...
	initrd_location = _initrd_start;

	if (!initrd.size && a2) {
		printf("Initrd passed by bootloader\n");
		initrd.size = a2;
		initrd_location = (void*)a1;
	}
	...
	if ( initrd.size > 0 ) {
		...
		printf("initial ramdisk moving 0x%lx <- 0x%lx (%lx bytes)\n\r",
		       initrd.addr, (unsigned long)initrd_location, initrd.size);
		memmove((void *)initrd.addr, (void *)initrd_location, initrd.size);
