...
static unsigned long mcnt;		/* # bytes to affect */
...
memops(int cmd)
{
	...
	switch( cmd ){
	case 'm':
		memmove((void *)mdest, (void *)msrc, mcnt);
		break;
	case 's':
		memset((void *)mdest, mval, mcnt);
		break;
	case 'd':
		if( termch != '\n' )
			termch = 0;
		scanhex((void *)&mdiffs);
		memdiffs((unsigned char *)mdest, (unsigned char *)msrc, mcnt, mdiffs);
		break;
	}
}
