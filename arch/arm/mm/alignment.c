...
do_alignment_ldrstr(unsigned long addr, unsigned long instr, struct pt_regs *regs)
{
	...
	if (LDST_L_BIT(instr)) {
		...
		get32_unaligned_check(val, addr);
		regs->uregs[rd] = val;
		/* 4.1.20 LDR ARMv5 and above fixups */
		if (rd == 15) {
			if (cpu_architecture() >= CPU_ARCH_ARMv5) {
				regs->uregs[rd] &= 0xfffffffe;
				if (val & 1)
					regs->ARM_cpsr |= PSR_T_BIT;
				else
					regs->ARM_cpsr &= ~PSR_T_BIT;
			} else
				regs->uregs[rd] &= 0xfffffffc;
		}
	} else
		put32_unaligned_check(regs->uregs[rd], addr);
	return TYPE_LDST;
...
do_alignment_ldmstm(unsigned long addr, unsigned long instr, struct pt_regs *regs)
{
	 ...
	 */
#if !(defined CONFIG_CPU_ARM922T)  && !(defined CONFIG_CPU_ARM920T)  && !(defined CONFIG_CPU_ARM925T)
	/*
		...
		for (regbits = REGMASK_BITS(instr), rd = 0; regbits;
			...
			if (regbits & 1) {
				if (LDST_L_BIT(instr)) {
					...
					regs->uregs[rd] = val;
					/* 4.1.17 LDM(1) ARMv5 and above fixups */
					if (rd == 15) {
						if (cpu_architecture() >= CPU_ARCH_ARMv5) {
							regs->uregs[rd] &= 0xfffffffe;
							if (val & 1)
