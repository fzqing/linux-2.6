...
static DEVICE_ATTR(name##_switch, S_IRUGO, gpio_sw_show_##name, NULL)
...
static int __init new_switch(struct gpio_switch *sw)
{
	...
	omap_set_gpio_direction(sw->gpio, 1);
	if (omap_get_gpio_datain(sw->gpio))
		omap_set_gpio_edge_ctrl(sw->gpio, OMAP_GPIO_FALLING_EDGE);
	else
		omap_set_gpio_edge_ctrl(sw->gpio, OMAP_GPIO_RISING_EDGE);
	r = request_irq(OMAP_GPIO_IRQ(sw->gpio), gpio_sw_irq_handler, SA_SHIRQ,
			sw->name, sw);
	if (r < 0) {
		printk(KERN_ERR "gpio-switch: request_irq() failed for GPIO %d\n", sw->gpio);
		platform_device_unregister(&sw->pdev);
