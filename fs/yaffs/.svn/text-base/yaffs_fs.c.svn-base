...
static struct super_block *yaffs_internal_read_super(int useRam, struct super_block * sb, void * data, int silent)
{
	...
	if(!sb)
		...
		T(YAFFS_TRACE_OS,("yaffs: yaffs_devname is NULL\n"));
	else
	...
	if(useRam)
	{
		...
		T(YAFFS_TRACE_OS,("yaffs: not RAM_ENABLED but useRam true"));
		return NULL;
#endif
...
#else
		/* Resolve compile warning that dev might not be initialized. */
		T(YAFFS_TRACE_OS,( "yaffs: not MTD_ENABLED but useRam false"));
		return NULL;
#endif
	}
