...
static raw_rwlock_t	rchan_table_lock = RAW_RW_LOCK_UNLOCKED;
 ...
 */
static void remove_rchan_file(void *private)
{
...
static void remove_rchan_file_tasklet(unsigned long private)
{
 ...
 */
static int relay_release(struct rchan *rchan)
{
	int err = 0;
		  ...
		  int interrupting)
{
	...
	if (deliver && 	waitqueue_active(&rchan->read_wait)) {
		err = relay_tasklet_schedule(&rchan->wake_readers_tasklet,
...
__relay_reset(struct rchan *rchan, int init)
{
	...
	if (init) {
		...
		relay_init_tasklets(rchan);
		atomic_set(&rchan->refcount, 0);
		INIT_LIST_HEAD(&rchan->open_readers);
		rchan->open_readers_lock = RAW_RW_LOCK_UNLOCKED;
	}
	   ...
	   u32 init_buf_size)
{
	...
	if ( (err = alloc_rchan_tasklets(rchan)) != 0)
		goto failed;
 ...
 *	Returns 0 if successful, negative otherwise.
 */
static int relay_finalize(int rchan_id)
{
	int err = 0;
 ...
 */
static void __relay_buffers_consumed(struct rchan *rchan, u32 bufs_consumed)
{
	int err;
	...
	err = relay_tasklet_schedule(&rchan->wake_writers_tasklet,
 ...
 */
static ssize_t
do_read(struct rchan *rchan, char *buf, size_t count, u32 read_offset, loff_t *new_offset, u32 *actual_read_offset)
{
 ...
 */
static ssize_t
__relay_read(struct rchan_reader *reader, char *buf, size_t count, u32 read_offset, loff_t *new_offset, u32 *actual_read_offset, int wait)
{
	int err = 0;
	size_t read_count = 0;
 ...
 */
ssize_t
relay_read(struct rchan_reader *reader, char *buf, size_t count, int wait, u32 *actual_read_offset, loff_t *new_offset)
{
	u32 read_offset;
	ssize_t read_count;
